...
#pragma comment(lib, "../../../../../build/AmbientLib.lib")
...
std::string ambientAPI::queryEPOC()
{
	ambientPtr plugin(m_plugin.lock());
	return plugin->queryEPOC();
}

int ambientAPI::initialize(int framesToSkip, unsigned int width, unsigned int height, float EPOC_BUFFER_SIZE_IN_SECS){

	ambientPtr plugin(m_plugin.lock());
	return plugin->initialize(framesToSkip, width, height, EPOC_BUFFER_SIZE_IN_SECS);
}
